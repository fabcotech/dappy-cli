...
module.exports.pushFile = async () => {
  ...
  const pushFileOnChain = async () => {
    ...
    const term = rholangFilesModuleTerm(publicKey, nonce, {
    ...
    const checkDataOnChain = async () => {
      ...
      log(
        `registryUri :       ${jsValue.filesRegistryUri.replace("rho:id:", "")}`
      );
      log(
        `registryUriEntry :  ${jsValue.entryRegistryUri.replace("rho:id:", "")}`
      );
      log(
        `address :           ${jsValue.filesRegistryUri.replace(
          "rho:id:",
          ""
        )}.${fileId}\n`
      );
