...
module.exports.pushDapp = async () => {
  ...
  const push = async () => {
    ...
    if (boxData === 'error: box not found') {
      ...
      boxId = await deployBox(httpUrlValidator, config.options.shardId, privateKey, publicKey, masterRegistryUri, boxId.slice(3));
    ...
    if (contractId.slice(0, 3) !== masterRegistryUri.slice(0, 3)) {
      throw new Error('Make sure your contractId has the right prefix that matches master : ' + masterRegistryUri.slice(0, 3))
    }
