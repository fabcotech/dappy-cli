...
module.exports.createHtmlWithTags = config => {
  ...
  // TODO change dappy:// to avoid confusion with the protocol
  if (config.manifest.cssLibraries) {
    config.manifest.cssLibraries.forEach(lib => {
      const headClosesIndex = html.indexOf("</head>");
      const cssTag = `<link rel="stylesheet" type="text/css" href="${`dappy://css/${lib}.css`}"></link>`;
      html =
  ...
  if (config.manifest.jsLibraries) {
    config.manifest.jsLibraries.forEach(lib => {
      const headClosesIndex = html.indexOf("</head>");
      const jsTag = `<script type="text/javascript" href="${`dappy://js/${lib}.js`}"></script>`;
      html =
