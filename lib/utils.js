...
module.exports.checkConfig = async (recordOrDapp) => {
  ...
  let config = {
    options: {
      ...
      masterRegistryUri: process.env.DAPPY_NAMES_MASTER_REGISTRY_URI,
      boxId: process.env.DAPPY_NAMES_BOX_ID,
      nameSystemContractId: process.env.DAPPY_NAMES_CONTRACT_ID,
...
module.exports.createHtmlWithTags = (config) => {
  ...
  if (css) {
    ...
    html = html.substr(0, headClosesIndex) + cssTag + html.substr(headClosesIndex);
  }
  ...
  if (js) {
    ...
    html = html.substr(0, headClosesIndex) + jsTag + html.substr(headClosesIndex);
  }
...
module.exports.deployBox = async (httpUrlValidator, privateKey, publicKey, masterRegistryUri, boxId) => {
