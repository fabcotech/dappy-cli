...
module.exports.buildDeployData = (code, timestamp, publicKey) => {
  // const toSign = DeployData.encode(clear(deployData)).finish();
  const toSign = {
    term: code,
    timestamp,
    deployer: null, // Buffer.from(publicKey, "hex"),
    sig: null,
  ...
  const a = DeployData.encode(toSign).finish();
  const hash = blake2b256Hash(Buffer.from('@"world"!("Hello!")'));
  console.log("hash", hash, hash.length);
  // sig (hash(term) + timestamp) using private key
