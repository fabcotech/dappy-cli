...
module.exports.createHtmlWithTags = config => {
  ...
  if (config.manifest.cssLibraries) {
    config.manifest.cssLibraries.forEach(lib => {
      const headClosesIndex = html.indexOf("</head>");
  ...
  if (config.manifest.jsLibraries) {
    config.manifest.jsLibraries.forEach(lib => {
      const headClosesIndex = html.indexOf("</head>");
      const jsTag = `<script type="text/javascript" src="${`dappyl://js/${lib}.js`}"></script>`;
      html =
...
module.exports.rholangFilesModuleTerm = (publicKey, nonce, addFile) => {
  let term = `new 
    filesModuleCh,
    filesModuleReaderCh,
    filesUriCh,
  ...
  in {
    ...
    insertArbitrary!(*filesModuleReaderCh, *filesUriCh) |

    for (ch <= filesModuleReaderCh) {
      for (current <<- filesModuleCh) {
        ch!(*current)
      }
    } |

    for (entryUri <- entryUriCh; filesUri <- filesUriCh) {ADD_FILE}
  }`;
...
module.exports.rholangFilesModuleResourceTerm = (registryUri, fileId) => {
...
module.exports.rholangFilesModuleBasketTerm = registryUri => {
  return `new return, filesModuleCh, readCh, lookup(\`rho:registry:lookup\`) in {
    lookup!(\`rho:id:${registryUri}\`, *filesModuleCh) |
    for(filesModuleReader <- filesModuleCh) {
      new x in {
...
module.exports.rholangFilesModuleAddResourceTerm = (
  registryUri,
  fileId,
  fileBase64,
  signature,
...
module.exports.generateSignatureForNonce = (nonce, privateKey) => {
