...
module.exports.update = async () => {
  ...
  const update = async () => {
    ...
    const exploreDeployResponse = await rchainToolkit.http.exploreDeploy(
      httpUrlReadOnly,
      {
        term:  rchainToken.readBagsTerm(registryUri)
      }
    );
    ...
    if (!jsValue[fileId] || !jsValue[fileId].nonce) {
      log(`Bag with ID ${fileId} does not exist or does not have .nonce value`, "error");
      console.log(jsValue);
      process.exit();
    }
    ...
    const nonce = jsValue[fileId].nonce;
    ...
    const term = rchainToken.updateBagDataTerm(
      registryUri,
      payload,
      signature,
    );
