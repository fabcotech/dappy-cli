...
module.exports.build = async () => {
  ...
  const createManifest = () => {
    ...
    if (htmlWithTags.includes("UNFORGEABLE_NAME_1")) {
      console.log(
        "CAREFUL, the dpy created by build script does not include the string UNFORGEABLE_NAME replaced by the previewed unforgeable name"
      );
    }
    ...
    const gzipped = zlib.gzipSync(dpy);

    fs.writeFileSync(name, gzipped.toString("utf-8"), err => {
      if (err) {
        console.error(err);
      }
    });
