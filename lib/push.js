...
const rchainToken = require('rchain-token-files');
...
module.exports.push = async () => {
  ...
  const push = async () => {
    ...
    term1 = rchainToken.mainTerm(uuidv4().replace(/-/g, ""), publicKey);
            ...
            rchainToolkit.http
              ...
              .then((dataAtNameResponse) => {
                ...
                if (!parsedResponse.exprs.length) {
                  log(
                    "Transaction data not found (rchain-token-files deployment), will retry in 15 seconds"
                  );
                  return;
                }
    ...
    htmlWithTags = htmlWithTags
      .replace(
        ...
        dataAtNameResponseExpr1.registryUri.replace("rho:id:", "")
      )
    ...
    const contractNonce = dataAtNameResponseExpr1.nonce;
    const payload = {
      nonce: contractNonce,
      bags: {
        [`${fileId}`]: {
    ...
    const signature = generateSignature(ba, privateKey);

    let term2 = rchainToken.createTokensTerm(
      dataAtNameResponseExpr1.registryUri.replace('rho:id:', ''),
      payload,
      signature,
    );
    ...
    const checkDataOnChain = async () => {
      ...
      try {
        exploreDeployResponse = await rchainToolkit.http.exploreDeploy(
          httpUrlReadOnly,
          {
            term: rchainToken.readBagOrTokenDataTerm(
              dataAtNameResponseExpr1.registryUri.replace(
                "rho:id:",
                ""
              ),
      ...
      if (jsValue2) {
        if (jsValue2 === dpy) {
          ...
          log(
            `registryUri :       ${dataAtNameResponseExpr1.registryUri.replace(
              "rho:id:",
              ""
            )}`
          );
          log(
            `address :           ${dataAtNameResponseExpr1.registryUri.replace(
              "rho:id:",
              ""
            )}.${fileId}\n`
          );
