...
module.exports.push = async () => {
  ...
  const push = async () => {
    ...
    if (!boxId) {
      ...
      const term = rchainToken.readBoxTerm({ masterRegistryUri: masterRegistryUri, boxId: boxId});
      const result0 = await rchainToolkit.http.exploreDeploy(httpUrlReadOnly, {
        term: term,
      });

      const boxData = rchainToolkit.utils.rhoValToJs(JSON.parse(result0).expr[0]);
      if (boxData === "error: box not found") {
        log("deploying box now");
        const term2 = rchainToken.deployBoxTerm({ publicKey: publicKey, boxId: boxId, masterRegistryUri: masterRegistryUri });
        const timestampBox = new Date().getTime();
        const pd = await rchainToolkit.http.prepareDeploy(httpUrlReadOnly, {
          deployer: publicKey,
          timestamp: timestampBox,
    ...
    const checkDataOnChain = async () => {
      ...
      if (jsValue2) {
        if (jsValue2[purseId] === dpy) {
          ...
          log(`full address :                     ${masterRegistryUri}.${contractId}.${purseId}`);
