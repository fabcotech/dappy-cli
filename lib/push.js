...
module.exports.push = async () => {
  ...
  const push = async () => {
    ...
    const term1 = rholangFilesModuleTerm(publicKey, uuidv4().replace(/-/g, ""));
    ...
    try {
      dataAtNameResponseExpr1 = await new Promise((resolve, reject) => {
        ...
        interval = setInterval(() => {
          i += 1;
          if (i > 24) {
            reject("6 minutes timeout exceeded");
            return;
          }
    ...
    if (term2.indexOf("NONCE") !== -1) {
      const nonce = uuidv4().replace(/-/g, "");
      log('Replaced "NONCE" (found in rholang), with ' + nonce);
      term2 = term2.replace(new RegExp("NONCE", "g"), nonce);
    }
