  ...
  const push = async () => {
            ...
            rchainToolkit.http
              ...
              .then(dataAtNameResponse => {
                ...
                if (!parsedResponse.exprs.length) {
                  log(
                    "Cannot retreive transaction data (files module deployment), will retry in 15 seconds"
                  );
                  return;
                }
    ...
    const checkDataOnChain = async () => {
      ...
      if (!parsedResponse || !parsedResponse.expr[0]) {
        log(
          "Cannot retreive transaction data (file upload), will retry in 15 seconds"
        );
        return;
      }
