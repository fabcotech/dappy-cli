...
subjectAltName=${hosts.map((h, ind) => `DNS.${ind + 1}:${h}`).join(',')}`, 'utf8'
      ...
      exec(
        ...
        (error, stdout, stderr) => {
          ...
          if (getUniqueProcessArgv('--apply')) {
            let s = `--domain ${getProcessArgv('--domain')}`;
            if (getProcessArgv('--hosts')) {
              s = `--hosts ${getProcessArgv('--hosts')}`;
            }
            exec(
              `${process.argv[0]} ${process.argv[1]} apply --cert group${i + 1}.crt ${s}`,
              { maxBuffer: 1024 * 1000 },
              (error2, stdout2, stderr2) => {
                if (error2) {
                  console.log(stdout2);
                  throw new Error('Error applying certificate to config');
                }
                console.log(stdout2);
                i += 1;
                if (groups[i]) {
