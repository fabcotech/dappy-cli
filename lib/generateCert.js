...
module.exports.generateCert = async () => {
  ...
  if (hostFromArgv.endsWith('.dappy') || hostFromArgv.endsWith('.d')) {
  ...
  const altNames = [hostFromArgv];
  if (!hostFromArgv.endsWith('.dappy') && !hostFromArgv.endsWith('.d')) {
    altNames.push(`${hostFromArgv}.d`)
  }

  pem.createCertificate(
    { days: days, selfSigned: true, altNames: altNames },
    function (err, keys) {
      if (err) {
        console.log(err);
        throw new Error('Failed to generate TLS certifcate')
      }
      console.log('Certificate is valid for hosts ' + altNames.join(', '));
